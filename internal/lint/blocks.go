...
func checkBlocksFile(r *runner, rel, start, end string) {
	...
	if !ok {
		r.red(ruleBlocks, rel, "path escapes the repository root")
		return
	}
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			r.red(ruleBlocks, rel, "file with a declared ownership block does not exist")
			return
		}
	...
	case len(starts) == 0 && len(ends) == 0:
		r.add(Finding{
			Rule: ruleBlocks, Severity: SeverityRed, Path: rel,
